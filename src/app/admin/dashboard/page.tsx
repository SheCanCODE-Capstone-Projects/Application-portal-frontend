...
export default function DashboardPage() {
  ...
  const filteredApplications = useMemo(() => {
    return applications
        .filter((app) => {
          const matchesSearch =
              (app.personalInfo?.fullName?.toLowerCase() || "").includes(
                  searchTerm.toLowerCase(),
              ) ||
              (app.personalInfo?.email?.toLowerCase() || "").includes(
                  searchTerm.toLowerCase(),
              ) ||
              app.id.toLowerCase().includes(searchTerm.toLowerCase());
          const matchesCohort =
              selectedCohort === "all" || app.cohortName === selectedCohort;
          const matchesStatus =
              selectedStatus === "all" || app.status === selectedStatus;
          const matchesGender =
              selectedGender === "all" ||
              app.personalInfo?.gender === selectedGender;
          const matchesYear =
              selectedYear === "all" ||
              new Date(app.createdAt).getFullYear().toString() === selectedYear;
        ...
        .sort((a, b) => {
          switch (sortOption) {
            case "name-asc":
              return (a.personalInfo?.fullName || "").localeCompare(
                  b.personalInfo?.fullName || "",
              );
            case "name-desc":
              return (b.personalInfo?.fullName || "").localeCompare(
  ...
  const rejectionRate = stats
      ? (stats.systemRejects / (stats.totalApplicants || 1)) * 100
      : 0;
  const syncIssueRate = stats
      ? (stats.duplicateRejections / (stats.totalApplicants || 1)) * 100
      : 0;
  ...
  return (
      <div className="p-6 space-y-8 max-w-[1800px] mx-auto animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 shadow-sm rounded-full border border-gray-100/50">
          <GraphicalCard
          ...
          {/* NEW: Synchronized Users Card */}
          <GraphicalCard
              title="Synchronized"
              value={stats?.synchronizedUsers || 0}
              icon={<RefreshCw className="text-teal-600 w-5 h-5" />}
              color="#0d9488"
              bg="bg-teal-50/50"
              subtext="Mapped to Master DB"
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Volume of new applications received per day
              </CardDescription>
            </CardHeader>
        ...
        <Card className="overflow-hidden border-0 shadow-md ring-1 ring-gray-200">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tr>
                  ...
                  <th className="px-6 py-4 font-semibold text-gray-600">Cohort Name</th>
                ...
                {paginatedApplications.length === 0 ? (
                    ...
                    paginatedApplications.map((app) => (
                        <tr key={app.id} className="hover:bg-gray-50/80 transition-colors">
                          ...
                          <td className="px-6 py-4"><div className="font-normal text-gray-600">{app.cohortName || "N/A"}</div></td>
                          <td className="px-6 py-4 text-gray-500">{app.submittedAt ? new Date(app.submittedAt).toLocaleString() : "Not Submitted"}</td>
              ...
              <div className="flex justify-between items-center px-6 py-4 border-t bg-white">
                ...
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="outline" disabled={currentPage === 1} onClick={() => setCurrentPage((prev) => prev - 1)}>Previous</Button>
                  <Button size="sm" variant="outline" disabled={currentPage === totalPages || totalPages === 0} onClick={() => setCurrentPage((prev) => prev + 1)}>Next</Button>
                </div>
              </div>
            </div>
          </CardContent>
          ...
          {chartData ? (
              ...
              <div className="flex flex-col justify-end h-full">
                <div className="flex justify-between text-xs text-gray-500 mb-1">
                  <span>Rate</span><span className="font-bold">{progress.toFixed(1)}%</span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-1000 ${isWarning ? "bg-orange-500" : "bg-rose-500"}`} style={{ width: `${progress}%` }}></div>
                </div>
              </div>
          ) : (
              <div className={`absolute -right-4 -bottom-6 w-24 h-24 rounded-full opacity-10 ${(bg ?? "").replace("/50", "")}`} />
          )}
        </div>
      </div>
  );
